...
<div class="max-w-7xl mx-auto px-6 pt-28 pb-20">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10 pb-6 border-b border-gray-100">
        ...
        <div class="flex items-center gap-2 bg-gray-100 p-1.5 rounded-2xl w-full md:w-auto">
            <button type="button" onclick="filterType('all')" class="px-5 py-2 rounded-xl text-sm font-bold transition-all {{ request('type_filter', 'all') === 'all' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-800' }}">
                All Properties
            </button>
            <button type="button" onclick="filterType('hotel')" class="px-5 py-2 rounded-xl text-sm font-bold transition-all {{ request('type_filter') === 'hotel' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-800' }}">
                🏨 Hotels
            </button>
            <button type="button" onclick="filterType('villa')" class="px-5 py-2 rounded-xl text-sm font-bold transition-all {{ request('type_filter') === 'villa' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-800' }}">
                🏡 Villas
            </button>
        </div>
...
document.getElementById('plus').onclick = function(e){
    e.preventDefault();
    if(guests < 10){ guests++; count.innerHTML = guests; input.value = guests; }
}
document.getElementById('minus').onclick = function(e){
    e.preventDefault();
    if(guests > 1){ guests--; count.innerHTML = guests; input.value = guests; }
}
