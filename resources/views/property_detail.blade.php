...
<body class="bg-background text-on-background antialiased">
    ...
    <script>
        ...
        function handleFormSubmit(e) {
            ...
            // UBAH BAGIAN ALERT INI
            if (!unitId) {
                Swal.fire({
                    icon: 'info',
                    title: 'Select a Unit',
                    text: 'Please select an available unit room first before continuing your reservation.',
                    confirmButtonColor: '#2563eb', // Warna biru 
                    confirmButtonText: 'OK'
                }).then((result) => {
                    if (result.isConfirmed) {
                        // Scroll ke bagian "Available Units"
                        const target = document.querySelector('.mt-10 h2.text-xl');
                        if (target) {
                            window.scrollTo({
                ...
                return; // Jangan hapus return ini
            }
